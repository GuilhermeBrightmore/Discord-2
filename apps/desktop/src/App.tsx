...
import { ChevronDown, Expand, Hash, HeadphoneOff, Headphones, MicOff, PhoneOff, Plus, Users, Volume2 } from "lucide-react";
...
import { useVoiceLobby } from "./hooks/useVoiceLobby";
import { loadConversations, loadMembers, loadWorkspace, type DirectConversation, type ServerBundle } from "./lib/data";
...
export function App() {
  ...
  const [serverSettings, setServerSettings] = useState<{ serverId: string; tab: "overview" | "channels"; kind?: "text" | "voice" }>();
  ...
  return <div className="app-shell">
    ...
    {voiceChannel && <VoiceStage key={voiceChannel.id} channel={voiceChannel} profile={profile} expanded={voiceExpanded} onMinimize={() => setVoiceExpanded(false)} onExpand={() => setVoiceExpanded(true)} onLeave={() => setVoiceChannel(null)} onSettings={() => setAppSettings(true)} />}
...
function ChannelSidebar({ server, profile, activeChannelId, voiceChannel, onChannel, onResumeCall, onLeaveCall, onSettings, onAppSettings, onLogout }: { server: ServerBundle; profile: Profile; activeChannelId?: string; voiceChannel: Channel | null; onChannel(channel: Channel): void; onResumeCall(): void; onLeaveCall(): void; onSettings(tab: "overview" | "channels", kind?: "text" | "voice"): void; onAppSettings(): void; onLogout(): void }) {
  ...
  return <aside className="channel-sidebar"><header className="server-header"><button className="server-settings-trigger" onClick={() => onSettings("overview")} title="Configuracoes do servidor"><strong>{server.name}</strong><ChevronDown /></button></header><div className="channel-scroll"><section><div className="section-label"><span>CANAIS DE TEXTO</span>{server.role !== "member" && <button onClick={() => onSettings("channels", "text")}><Plus /></button>}</div>{text.map((channel) => <button className={`channel-item ${activeChannelId === channel.id ? "active" : ""}`} key={channel.id} onClick={() => onChannel(channel)}><Hash /><span>{channel.name}</span></button>)}</section><section><div className="section-label"><span>CANAIS DE VOZ</span>{server.role !== "member" && <button onClick={() => onSettings("channels", "voice")}><Plus /></button>}</div>{voice.map((channel) => <div className={`voice-channel-block ${voiceChannel?.id === channel.id ? "connected" : ""}`} key={channel.id}><button className="channel-item voice" onClick={() => onChannel(channel)}><Volume2 /><span>{channel.name}</span>{voiceMembers[channel.id]?.length ? <small>{voiceMembers[channel.id]?.length ?? 0}</small> : null}</button>{voiceMembers[channel.id]?.map((member) => <div className="voice-member-row" key={member.userId}><div>{member.avatarUrl ? <img src={member.avatarUrl} alt="" /> : <span>{member.displayName.slice(0, 1).toUpperCase()}</span>}</div><strong>{member.displayName}</strong>{member.deafened ? <HeadphoneOff /> : member.muted ? <MicOff /> : <i className="voice-speaking-dot" />}</div>)}</div>)}</section></div>{connectedHere ? <div className="voice-status connected"><Headphones /><div><strong>Voz conectada</strong><small>{voiceChannel?.name}</small></div><button title="Abrir chamada" onClick={onResumeCall}><Expand /></button><button className="disconnect" title="Sair da chamada" onClick={onLeaveCall}><PhoneOff /></button></div> : <div className="voice-status"><Headphones /><div><strong>Voz pronta</strong><small>Clique em um canal</small></div></div>}<UserBar profile={profile} onSettings={onAppSettings} onLogout={onLogout} /></aside>;
}

function MemberPanel({ serverId, me }: { serverId: string; me: Profile }) {
  const [members, setMembers] = useState<Array<{ role: string; profile: Profile }>>([]);
  ...
  const memberRow = (member: { role: string; profile: Profile }, isOnline: boolean) => <div className={`member-row ${isOnline ? "" : "offline"}`} key={member.profile.id}><div className="member-avatar"><Avatar profile={member.profile} /><span className={isOnline ? "online-dot" : "offline-dot"} /></div><div><strong>{member.profile.displayName}</strong><small>{member.role === "owner" ? "Proprietario" : member.role === "admin" ? "Administrador" : `@${member.profile.username}`}</small></div></div>;
